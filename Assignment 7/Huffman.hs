...
convertToTip :: [(a,Int)] -> [(Btree a, Int)]
convertToTip [] = []
convertToTip (x:xs) = (Tip (fst x), snd x) : convertToTip xs 

sortTree :: [(Btree a, Int)]-> [(Btree a, Int)]
sortTree xs = sortOn snd xs

takeTwo :: [(Btree a, Int)]-> (Btree a, Int)
takeTwo (x:xs) = (Bin (fst x) (fst (head xs)), (snd x) + (snd(head xs)) ) 

buildTree :: [(Btree a, Int)]-> Btree a
buildTree [x] = fst x
buildTree xs = buildTree( sortTree(takeTwo xs : (drop 2 xs)))
